...
public final class ExperimentalFfmpegVideoRenderer extends DecoderVideoRenderer {
  ...
  private final boolean yieldToMediaCodec;
  ...
  public ExperimentalFfmpegVideoRenderer(
    ...
    this(
        ...
        numInputBuffers,
        numOutputBuffers,
        /* yieldToMediaCodec= */ false);
  }

  /**
   * Creates a new instance.
   *
   * @param yieldToMediaCodec Whether supported formats are reported as {@link
   *     C#FORMAT_EXCEEDS_CAPABILITIES} instead of {@link C#FORMAT_HANDLED}. A {@code MediaCodec}
   *     renderer placed before this one then keeps winning ties, so this renderer only takes over
   *     when no {@code MediaCodec} decoder exists for the format at all.
   */
  public ExperimentalFfmpegVideoRenderer(
      long allowedJoiningTimeMs,
      @Nullable Handler eventHandler,
      ...
      int numInputBuffers,
      int numOutputBuffers,
      boolean yieldToMediaCodec) {
    super(allowedJoiningTimeMs, eventHandler, eventListener, maxDroppedFramesToNotify);
    this.threads = threads;
    this.numInputBuffers = numInputBuffers;
    this.numOutputBuffers = numOutputBuffers;
    this.yieldToMediaCodec = yieldToMediaCodec;
  }
  ...
  public final @RendererCapabilities.Capabilities int supportsFormat(Format format) {
    ...
    if (!FfmpegLibrary.isAvailable() || !MimeTypes.isVideo(mimeType)) {
      ...
      return RendererCapabilities.create(
          yieldToMediaCodec ? C.FORMAT_EXCEEDS_CAPABILITIES : C.FORMAT_HANDLED,
